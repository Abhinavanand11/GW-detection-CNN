...
"""A program to calculate the false-alarm rate ,True alarm rate and sensitive
distance from a search algorithm. 
"""
import argparse
import numpy as np
import h5py
...
def get_stats(fgevents, bgevents, injparams, duration=None,
    ...
    ret['fg-far'] = fgfar
    logging.info('Calculating foreground TAR')
    wave_stats = tpevents[1].copy()      # stats of true positives
    wave_stats.sort()                    # ascending for searchsorted
    # For each θ = noise_stats[j], count TP with stat > θ
    fgtar = len(wave_stats) - np.searchsorted(wave_stats, noise_stats, side='right')
    fgtar = fgtar / duration
    ret['fg-tar'] = fgtar
